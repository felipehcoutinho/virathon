...
parser.add_argument("--abundance_max_reads", help="Set a maximum number of reads per sample to be mapped by parsed by bowtie2. All other reads are ignore. Default behavior is to use all reads in the sample", default=0, type=int)
...
def central():
    ...
    if (args.abundance_table == True):
        abundance_out_file = calc_abundance(merged_genomes_file,args.bowtiedb,args.metagenomes_dir,args.metagenomes_extension,args.abundance_max_reads)
...
def calc_abundance(genome_file,db_file,metagenomes_dir,metagenomes_extension,max_reads):
    prefix_genome_file = get_prefix(genome_file,args.in_format)
...
def index_samples_type1(metagenomes_dir,metagenomes_extension,count_reads):
    samples_index = defaultdict(dict)
    for dir in metagenomes_dir:
        files = glob.glob(f'{dir}/*{metagenomes_extension}')
        first_id = files[0]
...
def index_samples_type0(metagenomes_dir,metagenomes_extension,count_reads):
    samples_index = defaultdict(dict)
    for directory in metagenomes_dir:
        ...
        for file in files:
            if (re.search('_R1',file)):
                sample_id = file
                sample_id = re.sub('(.)*/','',sample_id)
                sample_id = re.sub(f'_R1(\.)+{metagenomes_extension}','',sample_id)
                pair = file
                pair = re.sub('_R1','_R2',pair)
                samples_index[sample_id]['R1'] = file
